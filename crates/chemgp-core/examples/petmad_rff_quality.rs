...
fn main() {
        ...
        match RpcOracle::new(&host, port, atomic_numbers.clone(), box_matrix) {
            ...
            Err(e) => {
                ...
                if attempts >= max_attempts {
                    panic!(
                        "Failed to connect to RPC server after {} attempts: {}",
                        max_attempts, e
                    );
                }
                eprintln!(
                    "RPC server not ready (attempt {}/{}), retrying in 2s...",
    ...
    eprintln!(
        "Generating {} training + {} test points...",
        n_train, n_test
    );
    ...
    eprintln!(
        "  {} test points evaluated ({} total oracle calls)",
        n_test,
        n_train + n_test
    );
    ...
    eprintln!(
        "Exact GP: energy MAE = {:.6} eV, gradient MAE = {:.6} eV/A",
    ...
    for &d_rff in &[
        ...
        eprintln!(
            "D_rff={:>4}: e_mae={:.6}, g_mae={:.6} (vs GP: e={:.6}, g={:.6})",
