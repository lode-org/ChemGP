...
pub fn write_con(path: &str, configs: &[MolConfig]) -> Result<(), String> {
    let mut writer =
        ConFrameWriter::from_path(path).map_err(|e| format!("Failed to create {}: {}", path, e))?;
    ...
    for cfg in configs {
        ...
        let mut builder = ConFrameBuilder::new(boxl, angles)
            .prebox_header(["Generated by ChemGP".to_string(), String::new()]);
...
pub fn write_neb_dat(
    ...
    writeln!(w, "img\trxn_coord\tenergy\tf_para").map_err(|e| format!("Write error: {}", e))?;
