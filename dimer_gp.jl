...
function check_interatomic_ratio(x_new::Vector{Float64}, X_train::Matrix{Float64}, 
                                  ratio_limit::Float64)
...
function rotate_dimer!(state::DimerState, model::GPModel, config::DimerConfig;
    ...
    for rot_iter in 1:config.max_rot_iter
        ...
        if dtheta < config.T_angle_rot
            verbose && println("  Rotation converged (θ = $(round(dtheta, digits=5)) < $(config.T_angle_rot))")
            break
        end
        ...
        verbose && println("  Rotation $rot_iter: θ = $(round(dtheta, digits=5)), |F_rot| = $(round(F_rot_norm, digits=5))")
    end
end
...
function gp_dimer_optimize(oracle_func, 
    ...
    config = DimerConfig(T_force_true, T_force_gp, T_angle_rot, trust_radius,
                        ratio_at_limit, max_outer_iter, max_inner_iter, 
                        max_rot_iter, alpha_trans)
    ...
    for outer_iter in 1:max_outer_iter
        ...
        k = MolInvDistSE(kernel_params.signal_var, kernel_params.inv_lengthscales,
                        kernel_params.frozen_coords, kernel_params.feature_map)
        ...
        for inner_iter in 1:max_inner_iter
            ...
            if inner_iter % 10 == 0 || inner_iter == 1
                verbose && @printf("  GP step %3d: E = %8.4f | |F| = %.5f | C = %+.3e | d_min = %.4f\n",
            ...
            if min_dist_new > trust_radius
                verbose && @printf("  ⚠ Trust radius exceeded (%.4f > %.4f)\n", 
                                  min_dist_new, trust_radius)
        ...
        verbose && @printf("  True: E = %8.4f | |F| = %.5f | C = %+.3e\n",
