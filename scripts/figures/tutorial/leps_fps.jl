...
perturbed_cols = [result.path.images[rand(rng, 1:length(result.path.images))] .+ 0.1 * randn(rng, 9) for _ in 1:n_needed]
...
selected_indices = farthest_point_sampling(remaining_mat, X_selected, n_select - 1; distance_fn = euclidean_dist)
...
ax = Axis(fig[1, 1];
    xlabel = L"$\mathrm{PC}_1$",
    ylabel = L"$\mathrm{PC}_2$")

# Pruned points (gray)
scatter!(ax, proj_pru[1, :], proj_pru[2, :];
    marker = :circle, markersize = 8,
    color = (:gray70, 0.6), strokecolor = :gray50, strokewidth = 0.5,
    label = "Pruned")

# Selected points (colored)
scatter!(ax, proj_sel[1, :], proj_sel[2, :];
    marker = :diamond, markersize = 10,
    color = RUHI.teal, strokecolor = :white, strokewidth = 1.0,
    label = "FPS selected")
