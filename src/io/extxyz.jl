...
const ELEMENT_SYMBOLS = Dict{Int,String}(
    ...
    41 => "Nb", 42 => "Mo", 44 => "Ru", 45 => "Rh", 46 => "Pd",
    47 => "Ag", 48 => "Cd", 49 => "In", 50 => "Sn", 53 => "I",
    54 => "Xe", 55 => "Cs", 56 => "Ba", 72 => "Hf", 73 => "Ta",
    74 => "W", 75 => "Re", 76 => "Os", 77 => "Ir", 78 => "Pt",
    79 => "Au", 80 => "Hg", 82 => "Pb", 83 => "Bi",
)
...
function _compute_f_para(path::NEBPath)
    ...
    for i in 1:n
        ...
        if i == 1 || i == n
            tau = (i == 1) ? (path.images[2] .- path.images[1]) :
        ...
        e_prev, e_curr, e_next = path.energies[i-1], path.energies[i], path.energies[i+1]
    ...
    open(filename, "w") do io
        for (i, image) in enumerate(path.images)
            ...
            for a in 1:n_atoms
                ...
                idx = 3*(a-1)
                @printf(io, "%-2s %20.15f %20.15f %20.15f %20.15f %20.15f %20.15f\n",
                    sym, image[idx+1], image[idx+2], image[idx+3],
                    -grad[idx+1], -grad[idx+2], -grad[idx+3])
            end
        end
    end
    return nothing
...
function write_neb_dat(path::NEBPath, filename::AbstractString)
    ...
    open(filename, "w") do io
        ...
        for i in 1:n
            @printf(io, "%3d %12.6f %12.6f %12.6f\n",
                i - 1, dists[i], path.energies[i] - E_ref, f_para[i])
        end
    end
    return nothing
end
...
function write_convergence_csv(result::NEBResult, filename::AbstractString)
    ...
    open(filename, "w") do io
        ...
        for i in 1:n
            max_f = h["max_force"][i]
            ci_f = haskey(h, "ci_force") && i <= length(h["ci_force"]) ? h["ci_force"][i] : 0.0
            oc = haskey(h, "oracle_calls") && i <= length(h["oracle_calls"]) ? h["oracle_calls"][i] : 0
            me = haskey(h, "max_energy") && i <= length(h["max_energy"]) ? h["max_energy"][i] : 0.0
...
function make_neb_writer(
    ...
    return function(path::NEBPath, iteration::Int)
        ...
        write_neb_trajectory(path, joinpath(output_dir, "neb_path_$idx.xyz"),
                             atomic_numbers, cell; pbc)
