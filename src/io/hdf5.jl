...
Layout:
```
/path/images        (n_images, ndof)     -- positions (row-major in Python/h5py)
/path/energies      (n_images,)          -- energies
/path/gradients     (n_images, ndof)     -- gradients (row-major in Python/h5py)
...
function write_neb_hdf5(
    ...
    h5open(filename, "w") do fid
        ...
        # NOTE: Julia is column-major, HDF5/h5py is row-major.
        # Writing (ndof, n_images) in Julia produces (n_images, ndof) in Python.
        g["images"] = collect(reduce(hcat, path.images))      # ndof x n_images
        g["energies"] = path.energies
        g["gradients"] = collect(reduce(hcat, path.gradients)) # ndof x n_images
        g["f_para"] = f_para
        g["rxn_coord"] = dists
...
f_para, and rxn_coord datasets, and updates `/path/` with the latest state.
The `/path/` group matches the layout of `write_neb_hdf5`, so the history
file is self-contained for both trajectory replay and final-state extraction
(e.g. `chemparseplot`'s `result_to_atoms_list`).
...
function make_neb_hdf5_writer(
    ...
    return function (path::NEBPath, iteration::Int)
        ...
        h5open(filename, mode) do fid
            ...
            imgs = collect(reduce(hcat, path.images))
            grads = collect(reduce(hcat, path.gradients))
            ...
            # Update /path with latest state (overwrite previous)
            haskey(fid, "path") && delete_object(fid, "path")
            gp = create_group(fid, "path")
            gp["images"] = imgs
            gp["energies"] = path.energies
            gp["gradients"] = grads
            gp["f_para"] = f_para
            gp["rxn_coord"] = dists
        end
